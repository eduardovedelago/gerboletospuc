...
$("#btnExecuteCancelamento").click(function (event) {
    event.stopImmediatePropagation();
    if (executaValidacaoDadosTela()) {
        executeCallCancelamento();
    } else {
...
function executeCallCancelamento() {
    var ope = $("#ed_operacao").val();
    var ope2 = $("#ed_operacao2").val();
    if (ope.trim() != "" && ope2 == ope) {
        var dataForm = {
            action: "executeCancelamento",
            operacao: ope,
            operacao2: ope2
        };
        $.ajax({
            type: 'post',
            url: '../controller/form-proc-cancel-blt-ctrl.php',
            data: dataForm,
            beforeSend: function () {
                $('body').loadingModal({text: 'Aguarde, Cancelando Movimento...'}).loadingModal('animation', 'foldingCube');
            },
            success: function (retornoAjax) {
                if (retornoAjax != null && retornoAjax == "OK") {
                    ...
                    $("#ed_operacao").focus();
                    $.bigBox({
                        title: "Cancelamento de Operação",
