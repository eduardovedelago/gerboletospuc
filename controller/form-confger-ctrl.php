...
// ##
// #####################################################
$entityName = "models\ConfGer";
$fieldNameInModal = "codigo";
$locationRedirAfter_Insert_Update = "./form-confger.php";
    ...
    if (($_POST['action'] == 'insert') || ($_POST['action'] == 'edit')) {
        ...
        try {
            if ($entityManager != null) {
                ...
                $entidade = null;
                if ($isInsert) {
                    $entidade = new ConfGer();
                    $entidade->setCodigo(0);
                } else {
                    $entidade = $entityManager->find($entityName, 1);
                }
                if ($entidade != null) {
                    $entidade->setTextoboleto($_POST['ed_textoboleto']);
...
// ##
// #####################################################
function validaDadosInsertUpdate(ConfGer $confGer, $isInsert)
{
    $msgDefault = "Validação de Dados: ";
    if ($confGer == null) {
        return $msgDefault . "Dados da Configuração Inválidos, Null";
    }
    if ($confGer->getTextoboleto() == null || $confGer->getTextoboleto() == "") {
        return $msgDefault . "Texto do Boleto inválido";
    }
    return null;
}
