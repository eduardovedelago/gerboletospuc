...
// ##
// #####################################################
$entityName = "models\PlanoGer";
$fieldNameInModal = "conta";
$locationRedirAfter_Insert_Update = "./form-cad-planoger.php";
        ...
        try {
            if ($entityManager != null) {
                ...
                if ($entidade != null) {
                    ...
                    $entidade->setContaContabil($_POST['edContaContabil']);
...
// ##
// #####################################################
function validaDadosInsertUpdate(PlanoGer $planoGer, $isInsert)
{
    $msgDefault = "Validação de Dados: ";
    if ($planoGer == null) {
        return $msgDefault . "Dados do Plano Gerencial Inválidos, Null";
    }
    if ($planoGer->getDescricao() == null || $planoGer->getDescricao() == "") {
